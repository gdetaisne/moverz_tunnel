(function () {
  ...
  function mountWidget(root, options) {
    ...
    style.textContent = [
      ...
      ".mzw-card { background: linear-gradient(135deg, #334155 0%, #1e293b 100%); border-radius: 24px; padding: 32px 28px 28px; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.40), 0 0 0 1px rgba(var(--mzw-spark), 0.30); color: #ffffff; max-width: 460px; display: flex; flex-direction: column; position: relative; overflow: hidden; }",
      ...
      ".mzw-badge { display: inline-flex; align-items: center; gap: 7px; padding: 6px 13px; border-radius: 999px; background: rgba(var(--mzw-spark), 0.22); border: 1px solid rgba(var(--mzw-spark), 0.45); color: rgb(var(--mzw-spark)); font-size: 11px; font-weight: 700; letter-spacing: 0.03em; box-shadow: 0 2px 8px rgba(var(--mzw-spark), 0.15); }",
      ".mzw-badge-dot { width: 7px; height: 7px; border-radius: 999px; background: rgb(var(--mzw-spark)); box-shadow: 0 0 8px rgba(var(--mzw-spark), 0.60); animation: mzw-pulse 2s ease-in-out infinite; }",
      "@keyframes mzw-pulse { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.7; transform: scale(0.95); } }",
      ".mzw-title { margin-top: 16px; font-size: 24px; font-weight: 800; line-height: 1.25; color: #ffffff; letter-spacing: -0.02em; }",
      ".mzw-subtitle { margin-top: 8px; font-size: 14px; color: #e2e8f0; line-height: 1.5; font-weight: 500; }",
      ".mzw-benefits { margin-top: 18px; display: flex; gap: 10px; }",
      ".mzw-benefit { display: flex; align-items: center; gap: 6px; padding: 7px 13px; border-radius: 999px; background: rgba(var(--mzw-spark), 0.18); border: 1px solid rgba(var(--mzw-spark), 0.40); font-size: 12px; color: rgb(var(--mzw-spark)); font-weight: 700; box-shadow: 0 2px 8px rgba(var(--mzw-spark), 0.10); transition: all 0.2s ease; }",
      ".mzw-benefit:hover { transform: translateY(-2px); background: rgba(var(--mzw-spark), 0.25); box-shadow: 0 4px 12px rgba(var(--mzw-spark), 0.18); }",
      ".mzw-benefit-icon { font-size: 11px; }",
      ".mzw-dropzone { margin-top: 0; border: 2px dashed rgba(var(--mzw-spark), 0.50); border-radius: 18px; background: rgba(var(--mzw-spark), 0.12); padding: 40px 28px; text-align: center; cursor: pointer; transition: all 0.25s cubic-bezier(0.4, 0, 0.2, 1); position: relative; box-shadow: 0 4px 16px rgba(var(--mzw-spark), 0.10); }",
      ".mzw-dropzone:hover { border-color: rgba(var(--mzw-spark), 0.80); background: rgba(var(--mzw-spark), 0.20); transform: translateY(-4px); box-shadow: 0 12px 32px rgba(var(--mzw-spark), 0.25); }",
      ".mzw-dropzone.mzw-dropzone--active { border-color: rgb(var(--mzw-spark)); background: rgba(var(--mzw-spark), 0.25); border-style: solid; box-shadow: 0 0 0 4px rgba(var(--mzw-spark), 0.30), 0 12px 32px rgba(var(--mzw-spark), 0.35); }",
      ".mzw-drop-icon { width: 64px; height: 64px; border-radius: 16px; background: linear-gradient(135deg, rgb(var(--mzw-deep)), rgb(var(--mzw-spark))); display: inline-flex; align-items: center; justify-content: center; margin: 0 auto 18px; box-shadow: 0 8px 24px rgba(var(--mzw-spark), 0.50), 0 0 20px rgba(var(--mzw-spark), 0.20); transition: transform 0.25s ease; }",
      ".mzw-dropzone:hover .mzw-drop-icon { transform: scale(1.05) rotate(2deg); box-shadow: 0 12px 32px rgba(var(--mzw-spark), 0.60), 0 0 24px rgba(var(--mzw-spark), 0.30); }",
      ".mzw-drop-icon-svg { width: 30px; height: 30px; color: #ffffff; }",
      ".mzw-drop-title { font-size: 17px; font-weight: 800; color: #ffffff; letter-spacing: -0.02em; }",
      ".mzw-drop-helper { margin-top: 6px; font-size: 13px; color: #e2e8f0; font-weight: 500; }",
      ...
      ".mzw-photos-row { margin-top: 20px; display: flex; gap: 8px; flex-wrap: wrap; }",
      ".mzw-photo-pill { border-radius: 999px; background: rgba(var(--mzw-spark), 0.20); border: 1px solid rgba(var(--mzw-spark), 0.40); padding: 7px 13px; font-size: 13px; display: inline-flex; align-items: center; gap: 8px; white-space: nowrap; color: rgb(var(--mzw-spark)); font-weight: 700; box-shadow: 0 2px 8px rgba(var(--mzw-spark), 0.15); transition: all 0.2s ease; }",
      ".mzw-photo-pill:hover { transform: translateY(-2px); background: rgba(var(--mzw-spark), 0.28); box-shadow: 0 4px 12px rgba(var(--mzw-spark), 0.22); }",
      ".mzw-photo-pill-count { width: 22px; height: 22px; border-radius: 999px; background: linear-gradient(135deg, rgb(var(--mzw-deep)), rgb(var(--mzw-spark))); display: flex; align-items: center; justify-content: center; font-size: 11px; color: #ffffff; font-weight: 800; box-shadow: 0 2px 8px rgba(var(--mzw-spark), 0.40); }",
      ".mzw-photo-pill-remove { cursor: pointer; opacity: 0.80; font-size: 16px; font-weight: 700; transition: all 0.2s ease; margin-left: 4px; color: #f1f5f9; }",
      ".mzw-photo-pill-remove:hover { opacity: 1; transform: scale(1.2) rotate(90deg); color: #f87171; }",
      ".mzw-error { margin-top: 16px; font-size: 13px; color: #fca5a5; font-weight: 700; padding: 14px 16px; background: rgba(220, 38, 38, 0.18); border-radius: 14px; border: 1px solid rgba(220, 38, 38, 0.35); }",
      ...
      ".mzw-link { appearance: none; border: none; background: transparent; padding: 0; margin: 0; font-size: 13px; color: #94a3b8; font-weight: 700; cursor: pointer; text-decoration: underline; text-decoration-thickness: 1.5px; text-underline-offset: 3px; transition: all 0.2s ease; }",
      ".mzw-link:hover { color: rgb(var(--mzw-spark)); text-decoration-thickness: 2px; transform: translateY(-1px); }",
      ...
      ".mzw-results { margin-top: 16px; border-radius: 18px; background: rgba(var(--mzw-spark), 0.12); border: 1px solid rgba(var(--mzw-spark), 0.30); padding: 18px; max-height: 360px; overflow: auto; color: #ffffff; box-shadow: 0 4px 16px rgba(var(--mzw-spark), 0.12); }",
      ".mzw-results-header { display: flex; justify-content: space-between; align-items: center; gap: 10px; margin-bottom: 16px; padding-bottom: 14px; border-bottom: 1px solid rgba(var(--mzw-spark), 0.25); }",
      ".mzw-results-title { font-size: 16px; font-weight: 800; color: #ffffff; }",
      ".mzw-results-pill { font-size: 12px; color: #ffffff; background: linear-gradient(135deg, rgb(var(--mzw-deep)), rgb(var(--mzw-spark))); border-radius: 999px; padding: 5px 13px; font-weight: 800; box-shadow: 0 6px 16px rgba(var(--mzw-spark), 0.45); }",
      ".mzw-results-list { display: flex; flex-direction: column; gap: 10px; }",
      ".mzw-item { border-radius: 14px; background: rgba(51, 65, 85, 0.50); padding: 14px 16px; border: 1px solid rgba(var(--mzw-spark), 0.25); transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); }",
      ".mzw-item:hover { border-color: rgba(var(--mzw-spark), 0.45); transform: translateY(-2px); background: rgba(51, 65, 85, 0.70); box-shadow: 0 4px 12px rgba(var(--mzw-spark), 0.15); }",
      ".mzw-item-header { display: flex; justify-content: space-between; gap: 10px; align-items: center; }",
      ".mzw-item-label { font-size: 14px; font-weight: 800; color: #ffffff; }",
      ".mzw-item-badges { display: flex; gap: 6px; align-items: center; flex-wrap: wrap; justify-content: flex-end; }",
      ".mzw-chip { border-radius: 999px; background: rgba(var(--mzw-spark), 0.18); padding: 4px 10px; font-size: 11px; color: rgb(var(--mzw-spark)); font-weight: 800; border: 1px solid rgba(var(--mzw-spark), 0.30); }",
      ".mzw-chip-outline { border-radius: 999px; border: 1px solid rgba(148, 163, 184, 0.35); padding: 4px 10px; font-size: 11px; color: #cbd5e1; background: rgba(51, 65, 85, 0.40); font-weight: 700; }",
      ".mzw-item-body { margin-top: 6px; font-size: 12px; color: #cbd5e1; display: flex; flex-direction: column; gap: 2px; font-weight: 500; }",
