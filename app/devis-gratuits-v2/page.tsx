...
// =========================================================
// V2 – Gamification légère : "Précision" (mobile-first)
// =========================================================

function computeV2PrecisionScore(input: {
  maxReachedStep: StepId;
  donePoints: number;
  totalPoints: number;
}): {
  score: number;
  label: "Faible" | "Correct" | "Élevé" | "Très élevé";
  remainingToNextLabel: number;
  nextGainPoints: number;
} {
  const donePoints = Math.max(0, Math.floor(input.donePoints || 0));
  const totalPoints = Math.max(1, Math.floor(input.totalPoints || 1));

  // Base dépendante des infos déjà saisies (contact/projet/prix)
  const base =
    input.maxReachedStep >= 3 ? 35 : input.maxReachedStep === 2 ? 25 : 15;

  // On limite l'impact mission à 65 points max (pour garder de la place à la base).
  const missionScore = Math.round((donePoints / totalPoints) * 65);
  const score = Math.min(100, base + missionScore);
  ...
  const nextThreshold =
    score < 50 ? 50 : score < 70 ? 70 : score < 85 ? 85 : 100;
  const remainingToNextLabel = Math.max(0, nextThreshold - score);

  // Prochaine action : prendre un "petit" meuble non couvert apporte typiquement 6–12 pts.
  // (valeur réelle affichée dans la mission; ici on met un fallback.)
  const nextGainPoints = 8;

  return { score, label, remainingToNextLabel, nextGainPoints };
}

function V2PrecisionDock(props: {
  maxReachedStep: StepId;
  donePoints: number;
  totalPoints: number;
  nextGainPoints: number;
  ...
  const { score, label, remainingToNextLabel } =
    computeV2PrecisionScore({
      maxReachedStep: props.maxReachedStep,
      donePoints: props.donePoints,
      totalPoints: props.totalPoints,
    });

  const hint =
    remainingToNextLabel <= 0
      ? "Bravo, précision maximale atteinte."
      : `Encore ${remainingToNextLabel} pts pour le palier suivant`;

  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-40">
      <div className="pointer-events-auto mx-auto max-w-3xl px-3 pb-[calc(env(safe-area-inset-bottom,0px)+12px)]">
        <div className="rounded-2xl border border-slate-700/70 bg-slate-950/90 p-3 shadow-lg shadow-slate-950/60 backdrop-blur">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                className="inline-flex items-center justify-center rounded-xl bg-emerald-400 px-4 py-2 text-xs font-semibold text-slate-950 shadow-md shadow-emerald-500/30 transition hover:bg-emerald-300"
              >
                Ajouter 1 photo (+{props.nextGainPoints} pts)
              </button>
              <button
                type="button"
                onClick={props.onGoToPhotos}
                className="inline-flex items-center justify-center rounded-xl border border-slate-700/70 bg-slate-950/30 px-4 py-2 text-xs font-semibold text-slate-200 hover:border-slate-500"
              >
                Voir la mission
              </button>
            </div>
          ...
          <div className="mt-2 flex items-center justify-between text-[11px] text-slate-400">
            <span>
              {props.donePoints} / {props.totalPoints} pts couverts
            </span>
            <span className="text-slate-500">Sans photos: fourchette plus large</span>
          </div>
        </div>
      </div>
    </div>
...
interface LocalUploadFile {
  ...
  photoId?: string;
  storageKey?: string;
  roomIndex?: number; // V2: guidage par pièce
}
...
function estimateBedrooms(housingType: HousingType): number {
  switch (housingType) {
    case "studio":
      return 0;
    case "t1":
...
function buildMissionForHousingType(housingType: HousingType): MissionRoom[] {
  ...
  const rooms: MissionRoom[] = [
    ...
    {
      id: "OTHER",
      label: "Fragile / valeur (optionnel)",
      items: [
        {
          id: "MIRROR_1",
          type: "MIRROR",
          label: "Miroir / tableau / œuvre",
          points: 4,
          requiredIndex: 1,
        },
        {
...
function DevisGratuitsPageInner() {
  ...
  const [hasPhotosAnswer, setHasPhotosAnswer] = useState<"pending" | "yes" | "no">("yes");
  const [showSkipPhotosConfirm, setShowSkipPhotosConfirm] = useState(false);
  const [isQuickScanning, setIsQuickScanning] = useState(false);
  const [quickItemsByPhotoId, setQuickItemsByPhotoId] = useState<
    Record<string, { types: string[]; labels: string[] }>
  >({});
  const [quickLastDetectedLabels, setQuickLastDetectedLabels] = useState<string[]>([]);
  const [activeMissionRoomIndex, setActiveMissionRoomIndex] = useState(0);
  ...
  const scrollToV2Step = (stepId: StepId) => {
    if (typeof window === "undefined") return;
    window.setTimeout(() => {
      const el = document.getElementById(`v2-step-${stepId}`);
  ...
  const handleAnalyzePhotos = async () => {
    ...
    try {
      ...
      const alreadyUploaded = localUploadFiles.filter(
        (f) => f.status === "uploaded" && f.photoId && f.storageKey
      );

      // Si tout est déjà uploadé (scan V2 auto), on peut analyser sans ré-uploader.
      const shouldUpload = pendingFiles.length > 0;

      if (!shouldUpload && analysisProcesses) {
        setIsUploadingPhotos(false);
        setIsAnalyzing(false);
        return;
      }

      const uploadedForProcessing: Array<{
        id: string;
        storageKey: string;
      ...
      setLocalUploadFiles((prev) =>
        ...
        uploadedForProcessing.push(
          ...alreadyUploaded.map((f) => ({
            id: f.photoId as string,
            storageKey: f.storageKey as string,
            originalFilename: f.file.name,
          }))
        );
        // Timer indicatif (≈ 3s/photo)
        const totalForTimer = uploadedForProcessing.length || 1;
        setAnalysisTargetSeconds(totalForTimer * 3);
      } else {
        throw new Error("Aucune photo disponible pour l'analyse.");
      }

      if (uploadedForProcessing.length > 0) {
  ...
  // V2 : scan automatique à chaque ajout de photo (upload → quick-items),
  // sans lancer l'inventaire complet (process2).
  const quickScanPendingPhotos = async () => {
    if (!leadId) return;
    const pending = localUploadFiles.filter((f) => f.status === "pending");
    ...
    try {
      ...
      const uploadRes = await uploadLeadPhotos(leadId, filesToUpload);

      // Mise à jour des statuts + conservation du storageKey pour les scans IA
      setLocalUploadFiles((prev) =>
        prev.map((f) => {
          const ok = uploadRes.success.find((s) => s.originalFilename === f.file.name);
          const ko = uploadRes.errors.find((e) => e.originalFilename === f.file.name);
          if (ok) {
            return {
              ...f,
              status: "uploaded",
      ...
      if (uploadRes.success.length > 0) {
        const quickRes = await fetch("/api/ai/quick-items", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            photos: uploadRes.success.map((p) => ({
              id: p.id,
              storageKey: p.storageKey,
              originalFilename: p.originalFilename,
            })),
        ...
        if (quickRes.ok) {
          const quickData = (await quickRes.json()) as {
            results: Array<{
              photoId: string;
          ...
          const nextByPhoto: Record<string, { types: string[]; labels: string[] }> =
            {};
          const lastLabels: string[] = [];
          for (const r of quickData.results ?? []) {
            nextByPhoto[r.photoId] = {
              types: Array.isArray(r.detectedTypes) ? r.detectedTypes : [],
              labels: Array.isArray(r.detectedLabels) ? r.detectedLabels : [],
            };
            lastLabels.push(...(Array.isArray(r.detectedLabels) ? r.detectedLabels : []));
          }
          setQuickItemsByPhotoId((prev) => ({ ...prev, ...nextByPhoto }));
          setQuickLastDetectedLabels(Array.from(new Set(lastLabels)).slice(0, 8));
        } else {
          console.warn("[V2][QuickItems] scan non disponible:", await quickRes.text());
        }
      }
    } catch (err: unknown) {
      const message =
        err instanceof Error ? err.message : "Erreur lors du scan rapide des photos.";
      console.warn("[V2][QuickItems] quick scan failed:", err);
      setError(message);
      setLocalUploadFiles((prev) =>
        prev.map((f) =>
          f.status === "uploading"
            ? { ...f, status: "error", error: "Échec upload/scan" }
            : f
        )
      );
    } finally {
  ...
  // Auto-scan dès qu'une nouvelle photo est ajoutée (mobile-first : feedback immédiat)
  useEffect(() => {
    if (currentStep !== 4) return;
    if (!leadId) return;
    if (photoFlowChoice !== "photos_now") return;
    if (isQuickScanning || isUploadingPhotos || isAnalyzing) return;
    const hasPending = localUploadFiles.some((f) => f.status === "pending");
    if (!hasPending) return;
    void quickScanPendingPhotos();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    currentStep,
    leadId,
    ...
    setLocalUploadFiles((prev) => {
      ...
      for (const file of array) {
        ...
        next.push({
          ...
          roomIndex: activeMissionRoomIndex,
        });
      }
      return next;
    });
  ...
  const resetTunnelV2 = () => {
    ...
    resetUploads();
    setQuickItemsByPhotoId({});
    setQuickLastDetectedLabels([]);
    setIsQuickScanning(false);
    setExcludedInventoryIds([]);
    setBackofficeLeadId(null);
    setLeadId("");
    setCurrentStep(1 as StepId);
  ...
  return (
    <div className="relative flex flex-1 flex-col gap-6 pb-40">
      ...
      <section
        id="v2-step-1"
        className="flex-1 rounded-2xl bg-slate-900/70 p-4 shadow-sm ring-1 ring-slate-800 sm:p-6"
      >
        <div className="mb-4 flex items-center justify-between gap-3">
          <button
            type="button"
            onClick={() => goToStep(1)}
            className="text-left text-sm font-semibold text-slate-50"
          >
            1. Contact
          </button>
          {currentStep !== 1 && (
            <button
              type="button"
              onClick={() => goToStep(1)}
              className="rounded-full border border-slate-700 bg-slate-950/40 px-3 py-1 text-[11px] font-semibold text-slate-200 hover:border-slate-500"
            >
              Modifier
            </button>
          )}
        ...
        {currentStep === 1 ? (
          ...
          <div className="space-y-1 text-sm text-slate-300">
            <p>
              <span className="font-semibold text-slate-200">Nom</span> :{" "}
            ...
            <p className="text-[11px] text-slate-500">
              Touchez “Modifier” pour changer vos infos.
            </p>
          </div>
        )}
      </section>
      ...
      >
        ...
        {currentStep === 2 ? (
          ...
          <div className="space-y-1 text-sm text-slate-300">
            <p>
              <span className="font-semibold text-slate-200">Départ</span> :{" "}
              {form.originCity?.trim()
                ? `${form.originPostalCode || "—"} ${form.originCity}`
                : "—"}
            </p>
            <p>
              <span className="font-semibold text-slate-200">Arrivée</span> :{" "}
              {form.destinationUnknown
                ? "Adresse non définie"
            ...
            <p>
              ...
              {form.movingDate ? formatMovingDateRange(form) : "—"}
            </p>
            <p className="text-[11px] text-slate-500">
              Touchez “Modifier” pour ajuster les adresses, accès et dates.
            </p>
          </div>
        )}
      </section>
      ...
      >
        ...
        {currentStep === 3 ? (
          ...
          <div className="space-y-1 text-sm text-slate-300">
            <p>
              <span className="font-semibold text-slate-200">Formule</span> :{" "}
              {form.formule}
            </p>
            <p>
              <span className="font-semibold text-slate-200">Fourchette</span> :{" "}
              {pricingByFormule?.[form.formule]
                ? `${formatPrice(pricingByFormule[form.formule]!.prixMin)} – ${formatPrice(
                    pricingByFormule[form.formule]!.prixMax
                  )}`
                : "—"}
            </p>
            <p className="text-[11px] text-slate-500">
              Touchez “Modifier” pour ajuster formule, options et estimation.
            </p>
          </div>
        )}
      </section>
      ...
      >
        ...
        {currentStep === 4 ? (
          <div className="space-y-6">
            {/* Mission photos (V2) : mobile-first, ludique, sans question binaire */}
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-[11px] text-slate-400">
                    Prenez surtout des photos des objets encombrants/lourds. Chaque photo augmente la précision.
                  </p>
                  {isQuickScanning && (
                    <p className="mt-2 text-[11px] font-medium text-sky-200">
                      Scan IA en cours…
                    </p>
                  )}
                  {!isQuickScanning && quickLastDetectedLabels.length > 0 && (
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px]">
                      <span className="font-semibold text-slate-200">Détecté :</span>
                      {quickLastDetectedLabels.map((label) => (
                        <span
                          key={label}
                          className="rounded-full border border-emerald-400/50 bg-emerald-500/10 px-2 py-0.5 font-semibold text-emerald-100"
                        >
                          {label}
                  </span>
                      ))}
                ...
                {/* CTA supprimé ici (redondant avec le dock + bouton principal ci-dessous). */}
              </div>

              <div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-800">
                <div
                  className="h-full rounded-full bg-emerald-400 transition-[width] duration-500"
                  style={{ width: `${v2Precision.score}%` }}
                />
              </div>
            ...
            {showSkipPhotosConfirm && (
              <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/70 px-3 pb-[calc(env(safe-area-inset-bottom,0px)+12px)] sm:items-center">
                <div className="w-full max-w-md space-y-3 rounded-2xl border border-slate-700 bg-slate-900 p-4 text-sm text-slate-100 shadow-xl">
                  <p className="text-sm font-semibold text-slate-50">
                    Continuer sans photos ?
                  </p>
                  <ul className="ml-4 list-disc space-y-1 text-[13px] text-slate-300">
                    <li>Fourchette plus large, risque de suppléments le jour J.</li>
                    <li>Inventaire à lister manuellement + visite/visio souvent demandée.</li>
                    <li>Objets fragiles/valeur moins bien couverts sans preuves.</li>
                  </ul>
                  <div className="flex flex-col gap-2 sm:flex-row">
                  <button
                    type="button"
                      onClick={() => setShowSkipPhotosConfirm(false)}
            ...
            {(hasPhotosAnswer === "yes" || photoFlowChoice !== "none") && (
              <>
            {/* En V2, le module "Mission" ci-dessus remplace l'intro binaire et le bloc explicatif long. */}
                  ...
                  {localUploadFiles.length > 0 && (
                    <div className="space-y-2 rounded-2xl bg-slate-950/70 p-3 text-xs text-slate-200">
                      ...
                      <div className="mt-3">
                        <button
                          type="button"
                          onClick={() => setShowFullInventoryPanel((v) => !v)}
                          className="inline-flex w-full items-center justify-center rounded-xl border border-slate-700 bg-slate-950/40 px-4 py-2.5 text-sm font-semibold text-slate-200 hover:border-slate-500"
                        >
                          {showFullInventoryPanel
                            ? "Masquer l’inventaire complet"
                            : "Inventaire complet (optionnel)"}
                        </button>

                        {showFullInventoryPanel && (
                          <div className="mt-3 space-y-2 rounded-2xl bg-slate-950/60 p-3 ring-1 ring-slate-800">
                            <p className="text-[11px] text-slate-400">
                              Pour un inventaire complet: prenez{" "}
                              <span className="font-semibold text-slate-200">
                                2–3 photos par pièce
                              </span>{" "}
                              (vue large + angles). Ensuite lancez l’analyse.
                            </p>
                        ...
                        <button
                          ...
                          disabled={
                            ...
                            isUploadingPhotos ||
                            isAnalyzing ||
                                  localUploadFiles.every(
                                    (f) => f.status !== "uploaded" && f.status !== "pending"
                                  )
                          }
          ...
          <div className="space-y-1 text-sm text-slate-300">
            <p>
              <span className="font-semibold text-slate-200">Photos ajoutées</span> :{" "}
              {localUploadFiles.length}
            </p>
            <p className="text-[11px] text-slate-500">
              Touchez “Modifier” pour ajouter des photos et augmenter la précision.
            </p>
          </div>
        )}
      </section>
      ...
      {isDev && (
        <div className="pointer-events-auto fixed bottom-3 left-3 z-50 flex items-center gap-2">
          <button
            type="button"
            onClick={resetTunnelV2}
            className="rounded-full border border-rose-400/60 bg-rose-950/30 px-3 py-1 text-[10px] font-semibold text-rose-200 shadow-sm shadow-slate-900/80 hover:border-rose-300 hover:text-rose-100"
          >
            Reset (debug)
          </button>
        <button
          ...
          onClick={async () => {
            ...
            className="rounded-full border border-slate-700/60 bg-slate-950/80 px-3 py-1 text-[10px] font-medium text-slate-400 shadow-sm shadow-slate-900/80 hover:border-sky-400 hover:text-sky-200"
        >
          Aller step 4 (debug)
        </button>
        </div>
