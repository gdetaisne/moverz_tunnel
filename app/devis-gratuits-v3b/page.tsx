...
function DevisGratuitsV3Content() {
  ...
  const source = searchParams.get("source") || searchParams.get("src") || "v3b";
  const from = searchParams.get("from") || "/devis-gratuits-v3b";
  ...
  useEffect(() => {
    if (state.currentStep === 1 && !state.leadId) {
      ...
      trackStep(1, "CONTACT", "contact_v3b");
    }
  }, [source, from]);
  ...
  useEffect(() => {
    const stepMap = {
      1: { logical: "CONTACT" as const, screen: "contact_v3b" },
      2: { logical: "PROJECT" as const, screen: "project_v3b" },
      3: { logical: "RECAP" as const, screen: "formules_v3b" },
      4: { logical: "THANK_YOU" as const, screen: "confirmation_v3b" },
    };
    ...
    if (state.currentStep === 1) {
      trackBlock("contact_info", "CONTACT", "contact_v3b");
    } else if (state.currentStep === 3) {
      trackBlock("surface_volume", "RECAP", "formules_v3b");
    } else if (state.currentStep === 4) {
      trackBlock("confirmation", "THANK_YOU", "confirmation_v3b");
    }
  }, [state.currentStep]);
  ...
  useEffect(() => {
    if (state.formule && !formulaTrackedRef.current && state.currentStep === 3) {
      ...
      trackBlock("formule", "RECAP", "formules_v3b");
    }
  ...
  useEffect(() => {
    if (state.currentStep === 2 && state.originHousingType && !originBlockTrackedRef.current) {
      ...
      trackBlock("origin_address", "PROJECT", "project_v3b");
    }
  ...
  useEffect(() => {
    if (state.currentStep === 2 && state.destinationHousingType && !destBlockTrackedRef.current) {
      ...
      trackBlock("destination_address", "PROJECT", "project_v3b");
    }
  ...
  useEffect(() => {
    if (state.currentStep === 2 && state.movingDate && !dateBlockTrackedRef.current) {
      ...
      trackBlock("moving_date", "PROJECT", "project_v3b");
    }
  ...
  const phoneCaptureTrackedRef = useRef(false);
  const hasAnyService = !!(
  ...
  useEffect(() => {
    if (state.currentStep === 3 && hasAnyService && !optionsBlockTrackedRef.current) {
      ...
      trackBlock("options", "RECAP", "formules_v3b");
    }
  ...
  async function handleSubmitStep1(e: FormEvent) {
    ...
    if (!state.firstName.trim() || state.firstName.trim().length < 2) {
      ...
      trackError("VALIDATION_ERROR", "Invalid firstName", 1, "CONTACT", "contact_v3b");
      return;
    }
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(state.email)) {
      ...
      trackError("VALIDATION_ERROR", "Invalid email", 1, "CONTACT", "contact_v3b");
      return;
    }
    ...
    try {
      ...
      trackStepChange(1, 2, "CONTACT", "PROJECT", "project_v3b", "forward");
      goToStep(2);
    } catch (err: any) {
      console.error("Error creating/updating lead:", err);
      trackError("API_ERROR", err.message || "Failed to create/update lead", 1, "CONTACT", "contact_v3b");
    }
  }
  ...
  async function handleSubmitStep2(e: FormEvent) {
    ...
    if (!isOriginValid || !isDestinationValid || !isDateValid) {
      ...
      trackError("VALIDATION_ERROR", "Invalid project fields", 2, "PROJECT", "project_v3b");
      return;
    }
    ...
    try {
      ...
      trackStepChange(2, 3, "PROJECT", "RECAP", "formules_v3b", "forward");
      ...
      trackError("API_ERROR", err.message || "Failed to update lead", 2, "PROJECT", "project_v3b");
    }
  }
  ...
  async function handleSubmitStep3(e: FormEvent) {
    ...
    if (originIsBoxStep3 && originBoxVolumeM3 == null) {
      ...
      trackError("VALIDATION_ERROR", "Missing exact box volume", 3, "RECAP", "formules_v3b");
      return;
    }
    ...
    if (surface < 10 || surface > 500) {
      ...
      trackError("VALIDATION_ERROR", "Invalid surface", 3, "RECAP", "formules_v3b");
      return;
    }
    ...
    if (!state.formule) {
      ...
      trackError("VALIDATION_ERROR", "No formule selected", 3, "RECAP", "formules_v3b");
      return;
    }
    ...
    try {
      ...
      trackStepChange(3, 4, "RECAP", "THANK_YOU", "confirmation_v3b", "forward");
      trackCompletion({ leadId: effectiveLeadId ?? null });
      setShowValidationStep3(false);
      goToStep(4);
    } catch (err: any) {
      console.error("Error finalizing lead:", err);
      trackError("API_ERROR", err.message || "Failed to finalize lead", 3, "RECAP", "formules_v3b");
    }
  }
        ...
        <div className="bg-white rounded-2xl md:rounded-3xl shadow-md md:shadow-lg p-4 sm:p-6 md:p-12">
          ...
          {state.currentStep === 4 && (
            <ConfirmationPage
              ...
              onPhoneChange={(v) => {
                updateField("phone", v);
                if (!phoneCaptureTrackedRef.current && v.trim().length > 0) {
                  phoneCaptureTrackedRef.current = true;
                  trackBlock("phone_capture", "THANK_YOU", "confirmation_v3b");
                }
              }}
              onGoToStep={(target) => {
                const stepMap = {
